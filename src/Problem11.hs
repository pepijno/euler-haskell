...
maxRow :: [[Int]] -> Int
maxRow = maximum . map (maximum . map product . filter ((==) 4 . length) . concat . map inits . tails) . filter ((<) 4 . length)

maxTotal :: [[Int]] -> Int
maxTotal xs = maximum [maxRow xs, maxRow $ transpose xs, maxRow diag1, maxRow diag2]
